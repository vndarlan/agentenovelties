...
def task_list_page():
    ...
    # Exibir tabela com opção de seleção
    ...
    # Verificar se alguma tarefa foi selecionada
    if selected_indices:
...
def task_detail_page():
    ...
    with col2:
        if status == 'running':
            ...
            if st.button("▶️ Executar Tarefa", use_container_width=True):
                # Executar a tarefa de forma assíncrona
    ...
    if status == 'created':
        ...
        placeholder.info("Iniciando execução da tarefa... Isso pode levar alguns minutos.")
        
        # Executar a tarefa assincronamente
        result = asyncio.run(execute_task(task_id))
        
        if result:
            placeholder.success("Tarefa concluída com sucesso!")
            st.experimental_rerun()
        else:
...
def main():
    ...
    with st.sidebar:
        ...
        nav_option = st.radio(
            "Navegação",
            ["Configuração", "Criar Tarefa", "Minhas Tarefas", "Detalhes da Tarefa" if st.session_state.current_task else None],
            index=3 if st.session_state.current_task else 0,
    ...
    if nav_option == "Configuração":
        auth_page()
    elif st.session_state.current_task and (nav_option == "Detalhes da Tarefa" or nav_option is None):
        task_detail_page()
